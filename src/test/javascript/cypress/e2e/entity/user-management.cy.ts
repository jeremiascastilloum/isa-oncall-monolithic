...
describe('UserManagement e2e test', () => {
  ...
  let username: string;
  let password: string;
  const userManagementSample = { login: 'Armando4', email: 'Rocio.RoqueZaragoza@hotmail.com' };
  ...
  describe('new UserManagement page', () => {
    ...
    it('should create an instance of UserManagement', () => {
      cy.get(`[data-cy="login"]`).type('Cesar66');
      cy.get(`[data-cy="login"]`).should('have.value', 'Cesar66');

      cy.get(`[data-cy="firstName"]`).type('María Luisa');
      cy.get(`[data-cy="firstName"]`).should('have.value', 'María Luisa');

      cy.get(`[data-cy="lastName"]`).type('Dávila Olivera');
      cy.get(`[data-cy="lastName"]`).should('have.value', 'Dávila Olivera');

      cy.get(`[data-cy="email"]`).type('Jordi31@yahoo.com');
      cy.get(`[data-cy="email"]`).should('have.value', 'Jordi31@yahoo.com');
