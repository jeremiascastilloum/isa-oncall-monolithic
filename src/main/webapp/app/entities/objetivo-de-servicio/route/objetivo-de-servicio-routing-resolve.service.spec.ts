...
import objetivoDeServicioResolve from './objetivo-de-servicio-routing-resolve.service';

describe('ObjetivoDeServicio routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IObjetivoDeServicio returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          objetivoDeServicioResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(objetivoDeServicioResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
