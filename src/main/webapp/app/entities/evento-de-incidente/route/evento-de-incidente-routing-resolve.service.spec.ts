...
import eventoDeIncidenteResolve from './evento-de-incidente-routing-resolve.service';

describe('EventoDeIncidente routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IEventoDeIncidente returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          eventoDeIncidenteResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(eventoDeIncidenteResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
