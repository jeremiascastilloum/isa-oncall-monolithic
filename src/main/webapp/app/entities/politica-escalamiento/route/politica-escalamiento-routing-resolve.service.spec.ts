...
import politicaEscalamientoResolve from './politica-escalamiento-routing-resolve.service';

describe('PoliticaEscalamiento routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IPoliticaEscalamiento returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          politicaEscalamientoResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(politicaEscalamientoResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
