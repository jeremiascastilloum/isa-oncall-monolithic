...
import { sampleWithRequiredData } from '../notificacion.test-samples';
...
describe('Notificacion Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 30327 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.notificacions()[0]).toEqual(expect.objectContaining({ id: 30327 }));
  });

  describe('trackId', () => {
    it('should forward to notificacionService', () => {
      const entity = { id: 30327 };
      vitest.spyOn(service, 'getNotificacionIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.notificacionsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should infinite scroll', async () => {
    ...
    req.flush([{ id: 30327 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
    expect(comp.notificacions()).toHaveLength(1);
    expect(comp.notificacions()[0]).toEqual(expect.objectContaining({ id: 30327 }));

    // WHEN
    comp.loadNextPage();
    TestBed.tick();
    expect(service.notificacionsParams()).toMatchObject(expect.objectContaining({ page: '1' }));
    ...
    await vitest.runAllTimersAsync();
    expect(comp.notificacions()).toHaveLength(2);
    expect(comp.notificacions()[1]).toEqual(expect.objectContaining({ id: 24975 }));

    comp.loadNextPage();
    TestBed.tick();
    expect(service.notificacionsParams()).toMatchObject(expect.objectContaining({ page: '2' }));
    req = httpMock.expectOne({ method: 'GET' });
    req.flush([{ id: 24975 }], {
      headers: { link: '<http://localhost/api/foo?page=0&size=20>; rel="prev",<http://localhost/api/foo?page=2&size=20>; rel="next"' },
    });
