...
import { sampleWithRequiredData } from '../equipo.test-samples';
...
describe('Equipo Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 20906 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.equipos()[0]).toEqual(expect.objectContaining({ id: 20906 }));
  });

  describe('trackId', () => {
    it('should forward to equipoService', () => {
      const entity = { id: 20906 };
      vitest.spyOn(service, 'getEquipoIdentifier');
      const id = comp.trackId(entity);
      expect(service.getEquipoIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.equiposParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
