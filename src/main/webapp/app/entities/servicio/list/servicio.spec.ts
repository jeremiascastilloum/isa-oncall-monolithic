...
import { sampleWithRequiredData } from '../servicio.test-samples';
...
describe('Servicio Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 24037 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.servicios()[0]).toEqual(expect.objectContaining({ id: 24037 }));
  });

  describe('trackId', () => {
    it('should forward to servicioService', () => {
      const entity = { id: 24037 };
      vitest.spyOn(service, 'getServicioIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.serviciosParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
