...
import { sampleWithRequiredData } from '../rotacion.test-samples';
...
describe('Rotacion Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: Rotacion;
  let fixture: ComponentFixture<Rotacion>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(Rotacion);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 23556 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.rotacions()[0]).toEqual(expect.objectContaining({ id: 23556 }));
  });

  describe('trackId', () => {
    it('should forward to rotacionService', () => {
      const entity = { id: 23556 };
      vitest.spyOn(service, 'getRotacionIdentifier');
      const id = comp.trackId(entity);
      expect(service.getRotacionIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.rotacionsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
