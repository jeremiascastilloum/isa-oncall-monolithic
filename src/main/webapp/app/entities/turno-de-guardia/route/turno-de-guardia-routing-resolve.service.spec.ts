...
import turnoDeGuardiaResolve from './turno-de-guardia-routing-resolve.service';

describe('TurnoDeGuardia routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return ITurnoDeGuardia returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          turnoDeGuardiaResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(turnoDeGuardiaResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
