...
import pasoEscalamientoResolve from './paso-escalamiento-routing-resolve.service';

describe('PasoEscalamiento routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IPasoEscalamiento returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          pasoEscalamientoResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(pasoEscalamientoResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
