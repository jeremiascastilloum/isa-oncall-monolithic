...
import { sampleWithRequiredData } from '../postmortem.test-samples';
...
describe('Postmortem Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: Postmortem;
  let fixture: ComponentFixture<Postmortem>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(Postmortem);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 24266 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.postmortems()[0]).toEqual(expect.objectContaining({ id: 24266 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 24266 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.postmortems()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 24266 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to postmortemService', () => {
      ...
      vitest.spyOn(service, 'getPostmortemIdentifier');
      const id = comp.trackId(entity);
      expect(service.getPostmortemIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.postmortemsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
