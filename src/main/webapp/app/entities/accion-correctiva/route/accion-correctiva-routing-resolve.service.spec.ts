...
import accionCorrectivaResolve from './accion-correctiva-routing-resolve.service';

describe('AccionCorrectiva routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IAccionCorrectiva returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          accionCorrectivaResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(accionCorrectivaResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
